...
def yolo_loss(args, anchors, num_classes, iou_thresh=0.5, print_loss=True):
    ...
    for i in range(y_num):
        ...
        raw_true_wh = backend.log(
            y_true[i][..., 2:4] * input_shape[::-1] / anchors[anchor_indices[i]])  # 计算y_pred_hw的真值，y_true_hw为归一化值
        ...
        xy_loss = true_mask * box_loss_scale * backend.binary_crossentropy(raw_true_xy, y_pred[..., 0:2],
                                                                           from_logits=True)
        ...
        confidence_loss = true_mask * backend.binary_crossentropy(true_mask, y_pred[..., 4:5], from_logits=True) + \
                          (1 - true_mask) * backend.binary_crossentropy(true_mask, y_pred[..., 4:5],
