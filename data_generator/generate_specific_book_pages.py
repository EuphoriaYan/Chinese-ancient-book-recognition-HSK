...
from data_generator.generate_text_lines import check_text_type, generate_mix_rows_chars_with_text, \
    generate_mix_cols_chars_with_text
...
def generate_book_page_with_text(obj_num=10, text_type="horizontal", init_num=0, page_shape=None, text='野火烧不尽春风吹又生'):
    ...
    with open(book_page_tags_file, "w", encoding="utf-8") as fw:
        for i in range(init_num, init_num + obj_num):
            ...
            PIL_page, text_bbox_list, text_list = create_book_page_with_text(_shape, text_type=text_type, text=text)
            image_tags = {"text_bbox_list": text_bbox_list, }
...
def create_book_page_with_text(shape=(960, 540), text_type="horizontal", text='野火烧不尽春风吹又生'):
    text_type = check_text_type(text_type)
    ...
    if text_type == "h":  # 横向排列
        ...
        for i in range(len(ys) - 1):
            ...
            _, text_bbox_list, _ = generate_mix_rows_chars_with_text(x, y1, y2, row_length, np_page, char_spacing)
        ...
        for i in range(len(xs) - 1, 0, -1):
            ...
            _, text_bbox_list, _ = generate_mix_cols_chars_with_text(x1, x2, y, col_length, np_page, char_spacing)
            text_bbox_records_list.extend(text_bbox_list)
    ...
    return PIL_page, text_bbox_records_list
