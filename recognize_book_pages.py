...
def load_segment_model(segment_task="book_page", text_type="vertical", model_struc="densenet_gru", weights=""):
    segment_model = segment_work_net(stage="predict", segment_task=segment_task, text_type=text_type,
...
class SegmentModel:
    ...
    def __init__(self, segment_task, text_type="vertical", model_struc="densenet_gru", weights=""):
        ...
        _model, self.weights_path = load_segment_model(self.segment_task, self.text_type, self.model_struc,
                                                       self.weights)
    ...
    for i in range(len(np_page_list)):
        try:
            ...
            np_line_list, page_split_lines = \
                extract_slices(np_page_list[i], page_split_pos_list[i], start_coord, segment_task="book_page",
                               text_type=text_type)
            ...
            if text_type in ("v", "vertical") and is_mix_line:
                ...
                for j in range(len(np_mix_line_list)):
                    np_double_line_list, mix_split_lines = \
                        extract_slices(np_mix_line_list[j], mix_split_pos_list[j], page_split_lines[j + 1, :2],
                                       segment_task="mix_line", text_type=text_type)
                    ...
                    double_split_pos_list, double_scores_list = check_and_correct_double_split(double_split_pos_list,
                                                                                               double_scores_list,
                                                                                               img_w)
                    text1, text2 = "", ""
                    for k in range(len(np_double_line_list)):
                        np_text_line_list, double_split_lines = \
                            extract_slices(np_double_line_list[k], double_split_pos_list[k], mix_split_lines[k, :2],
                                           segment_task="double_line", text_type=text_type)
                        ...
                        for t in range(len(np_text_line_list)):
                            np_char_list, single_split_lines = \
                                extract_slices(np_text_line_list[t], char_split_pos_list[t],
                                               double_split_lines[t + 1, :2], segment_task="text_line",
                                               text_type=text_type)
                ...
                for t in range(len(np_text_line_list)):
                    ...
                    np_char_list, single_split_lines = \
                        extract_slices(np_text_line_list[t], char_split_pos_list[t], page_split_lines[_t, :2],
                                       segment_task="text_line", text_type=text_type)
                    split_line_dict["single"].append(single_split_lines)
...
if __name__ == '__main__':
    ...
    parse.add_argument("--mix_line", type=bool, default=True,
                       help="Whether or not the text is single-double line type.")
