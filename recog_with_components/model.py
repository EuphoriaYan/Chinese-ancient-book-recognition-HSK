...
def summary_fn(inputs, **kwargs):
    ...
    sc_top3 = tf.reduce_mean(
        tf.cast(tf.reduce_any(pred_sc_labels[:, :3] == sc_labels[:, tf.newaxis], axis=1), tf.float32))
    sc_top5 = tf.reduce_mean(
        tf.cast(tf.reduce_any(pred_sc_labels[:, :5] == sc_labels[:, tf.newaxis], axis=1), tf.float32))
    ...
    lr_top3 = tf.reduce_mean(
        tf.cast(tf.reduce_any(tf.reduce_all(pred_lr_compo_seq[:, :3] == exp_lr_compo_seq, axis=2), axis=1), tf.float32))
    lr_top5 = tf.reduce_mean(
        tf.cast(tf.reduce_any(tf.reduce_all(pred_lr_compo_seq[:, :5] == exp_lr_compo_seq, axis=2), axis=1), tf.float32))
    ...
    correct_lr_acc = tf.reduce_mean(
        tf.cast(tf.reduce_all(correct_lr_compo_seq[:, 0] == lr_compo_seq, axis=1), tf.float32))
    correct_lr_top3 = tf.reduce_mean(
        tf.cast(tf.reduce_any(tf.reduce_all(correct_lr_compo_seq[:, :3] == exp_lr_compo_seq, axis=2), axis=1),
                tf.float32))
    correct_lr_top5 = tf.reduce_mean(
        tf.cast(tf.reduce_any(tf.reduce_all(correct_lr_compo_seq[:, :5] == exp_lr_compo_seq, axis=2), axis=1),
                tf.float32))
