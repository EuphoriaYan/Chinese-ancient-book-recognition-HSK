...
def adjust_img_to_fixed_height(np_img, split_positions=None, fixed_h=560, segment_task="book_page",
...
def adjust_img_to_fixed_height_tf(img_tensor, split_positions, fixed_h=560, segment_task="book_page",
                                  text_type="horizontal"):
    # rotate 90 degrees rightward.
    text_type = text_type[0].lower()
    if (segment_task, text_type) in (("book_page", "h"), ("double_line", "h"), ("text_line", "v"), ("mix_line", "v")):
...
def data_generator_with_images(annotation_lines,
    ...
    while True:
        ...
        for _ in range(batch_size):
            ...
            np_img, split_positions, _ = adjust_img_to_fixed_height(np_img, split_positions, fixed_h, segment_task,
                                                                    text_type)
            images_list.append(np_img)
            split_pos_list.append(split_positions)
            i = (i + 1) % n

        batch_imgs, real_images_width, split_positions = pack_a_batch(images_list, split_pos_list, feat_stride,
                                                                      background="white")
        inputs_dict = {"batch_images": batch_imgs, "real_images_width": real_images_width,
                       "split_lines_pos": split_positions}
...
def data_generator_with_tfrecords(tfrecords_files,
    ...
    def _parse_fn(serialized_example):
        ...
        img_tensor, split_positions = adjust_img_to_fixed_height_tf(img_tensor, split_positions, fixed_h, segment_task,
                                                                    text_type)
    ...
    def _image_augmentation(batch_imgs, real_images_width, split_positions):
        ...
        batch_imgs = tf.pad(batch_imgs, [[0, 0], [0, 0], [0, num_padding], [0, 0]], mode="CONSTANT",
                            constant_values=255)
...
def data_generator(data_file, src_type="images", segment_task="book_page", text_type="horizontal",
    ...
    if src_type == "images":
        training_generator = data_generator_with_images(train_lines, batch_size, fixed_h, feat_stride, segment_task,
                                                        text_type)
        validation_generator = data_generator_with_images(validation_lines, batch_size, fixed_h, feat_stride,
                                                          segment_task, text_type)
    elif src_type == "tfrecords":
        training_generator = data_generator_with_tfrecords(train_lines, batch_size, fixed_h, feat_stride, segment_task,
                                                           text_type)
        validation_generator = data_generator_with_tfrecords(validation_lines, batch_size, fixed_h, feat_stride,
                                                             segment_task, text_type)
    else:
